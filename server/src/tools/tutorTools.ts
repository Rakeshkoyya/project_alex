...
import { ARTIFACT_KINDS, parkedGenerations } from "../agents/generations.js";
...
export function tutorTools(ctx: RoleContext) {
  return [
    tool("get_learner_state", "Read learner state", "The student's current mastery per concept, ZPD frontier, today's plan, due flashcards, recent diary and last assessment.", Type.Object({}), () => json(learnerState(ctx))),
    tool(
      "set_today_plan",
      "Set today's plan",
    ...
    tool("complete_plan_item", "Tick plan item", "Mark an item of today's plan as done.", Type.Object({ planItemId: Type.String() }), ({ planItemId }) => {
      ctx.store.update(ctx.courseId, () => {
    ...
    tool("set_focus", "Focus concept", "Tell the UI which concept is being taught right now.", Type.Object({ conceptId: Type.String() }), ({ conceptId }) => {
    ...
    tool(
      ...
      ({ conceptId, correct, hintLevel, observation }) => {
    ...
    tool(
      ...
      ({ forConceptId, title, description }) => {
    ...
    tool("write_diary", "Write student diary", "Append an entry to the student's diary: observations, breakthroughs, struggles, how they like to learn.", Type.Object({ text: Type.String() }), ({ text }) => {
    ...
    tool(
      ...
      async ({ conceptIds, questionCount }) => {
